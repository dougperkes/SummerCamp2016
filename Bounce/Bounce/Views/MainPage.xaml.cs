...
namespace Bounce.Views
{
    public sealed partial class MainPage : Page
    {
        ...
        private async void verifyButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var isVIP = await IdentifyUserInGroup(file, vipGroupId, "VIP", true);
            if (isVIP)
            {
                vipBox.Visibility = Visibility.Visible;
                await Task.Delay(3000);
                vipBox.Visibility = Visibility.Collapsed;
            }
            var isCriminal = await IdentifyUserInGroup(file, criminalGroupId, "Criminal", false);
            if (isCriminal)
            {
                bannedBox.Visibility = Visibility.Visible;
                await Task.Delay(3000);
                bannedBox.Visibility = Visibility.Collapsed;
            }
                ...
                try
                {
                    ...
                    foreach (var identifyResult in results)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Log(string.Format("Identified as {0} in banned group. Acess Denied to enter", person.Name, groupLabel));
        ...
        private async Task CheckForFace(StorageFile imageToVerify)
        {
            ...
            using (var s = (await imageToVerify.OpenStreamForReadAsync()))
            {
                try
                {
                    ...
                    if (faces == null)
                    {
                        Log("No Faces were identified");
                        return;
                    } else
                    {
                        Log($"{faces.Count()} face(s) were identified.");
                    }
                    ...
                    foreach (Microsoft.ProjectOxford.Face.Contract.Face face in faces)
                    {
                        ...
                        Log($"Person {counter}: {gender}, age {age}. Glasses: {glasses}");
                        //Log(String.Format("Face Attributes: {0} is {1} years old, {0}, {2}, {3}, {4}, {6}, and is {5} ", heshe, age, hasBeard, hasMustache, hasSideburns, smiling, hasGlases, faceName));
